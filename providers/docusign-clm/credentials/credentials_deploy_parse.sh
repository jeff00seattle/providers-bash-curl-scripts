...
HQTEST3_CLIENT_ID=`cat ../credentials/config/credentials.deploy.json | jq '.hqtest3.client_id'`
HQTEST3_REDIRECT_URI=`cat ../credentials/config/credentials.deploy.json | jq '.hqtest3.redirect_uri'`
HQTEST3_AUTH_URL=`cat ../credentials/config/credentials.deploy.json | jq '.hqtest3.auth_url'`

HQTEST3_CLIENT_ID=`echo ${HQTEST3_CLIENT_ID} | tr -d \"`
HQTEST3_REDIRECT_URI=`echo ${HQTEST3_REDIRECT_URI} | tr -d \"`
HQTEST3_AUTH_URL=`echo ${HQTEST3_AUTH_URL} | tr -d \"`
...
if [ ${VERBOSE} = true ]
  then
    ...
    echo -e ""
    echo "  HQTEST3_CLIENT_ID:      ${HQTEST3_CLIENT_ID}"
    echo "  HQTEST3_REDIRECT_URI:   ${HQTEST3_REDIRECT_URI}"
    echo "  HQTEST3_AUTH_URL:       ${HQTEST3_AUTH_URL}"
    echo -e ""
