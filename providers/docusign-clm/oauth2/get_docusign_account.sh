...
# read the options
OPTS=`getopt -o hv --long help,verbose -n 'get_docusign_account.sh' -- "$@"`
...
rm -f ../credentials/config/credentials.docusign.account.json

echo "${HTTP_BODY}" |
  jq '.' > ../credentials/config/credentials.docusign.account.json
