...
The following bash scripts makes curl call to DocuSign SpringCM API and all expect a valid `[** ACCESS_TOKEN **]`:
...
To use these bash scripts requires a valid `[** ACCESS_TOKEN **]`. This can be
...
#### Add `authorization_code` to `credentials_code.json`
...
#### Add Auth Token JSON to `credentials_token.json`
