...
# read the options
OPTS=`getopt -o hv --long help,verbose,env:,id: -n 'envelope_void.sh' -- "$@"`
...
if [ -z "${ENVELOPE_ID}" ]
  then
    echo "Missing ENVELOPE_ID"; exit 1
fi
...
ESIGN_API_ENVELOPES_URL="${ESIGN_API_URL}/${ESIGN_API_VERSION}/accounts/${ACCOUNT_ID}/envelopes/${ENVELOPE_ID}"
...
curl --header "Authorization: Bearer ${ACCESS_TOKEN}" \
     ...
     --request PUT ${ESIGN_API_ENVELOPES_URL} \
     --data "{ \"status\": \"voided\", \"voidedReason\": \"The reason for voiding the envelope\" }" \
     --output ${response} \
     --verbose
